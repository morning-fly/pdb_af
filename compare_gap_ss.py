...
def main():
    ...
    results_df.to_csv("ss_gap_allchains.csv", index=False)
...
def main_flow(pdb_dir_path: Union[str, Path], af_dir_path: Union[str, Path], row_dict: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        trimmed_residues_list = [r.get_id()[1] for r in trimmed_residues]
        gap_residue_dict = get_outer_gap(trimmed_residues_list, len(af_chain))

        # 7. Analyze secondary structure for both experimental and AlphaFold (trimmed) chains
        af_gap_helix = analyze_is_helix_in_gap(af_chain, pdb_id, "af", gap_residue_dict)
...
def get_outer_gap(residue_list: List[int], aa_len: int) -> Dict[int, int]:
    """
    Calculate gap indicators for residues in a sequence.
    
    For each residue in `residue_list`, this function checks whether the adjacent 
    residues (i - 1 and i + 1) are missing while being within the valid range (1 to aa_len).
    
    The returned dictionary maps each residue with a single missing neighbor to an 
    integer value:
      - -1 indicates that only the left neighbor (i - 1) is missing.
      -  1 indicates that only the right neighbor (i + 1) is missing.
    
    If both neighbors (i - 1 and i + 1) are missing, a warning is logged and the residue
    is omitted from the returned dictionary.
    
    Args:
        residue_list (List[int]): List of existing residue numbers.
        aa_len (int): Total number of residues (i.e., maximum residue number).
    
    Returns:
        Dict[int, int]: A dictionary mapping each residue with exactly one missing neighbor 
                        to an integer gap indicator.
    """
    # Convert residue_list to a set for fast membership checking.
    residue_set = set(residue_list)
    gap_dict: Dict[int, int] = {}

    for i in residue_list:
        # Check if the left neighbor is missing and within the valid range.
        missing_left = (i - 1 not in residue_set) and (1 <= i - 1 <= aa_len)
        # Check if the right neighbor is missing and within the valid range.
        missing_right = (i + 1 not in residue_set) and (1 <= i + 1 <= aa_len)
        
        # If both neighbors are missing, log a warning and skip this residue.
        if missing_left and missing_right:
            logger.warning(f"Residue {i} has both missing neighbors (i-1 and i+1); skipping.")
            continue
        
        # If only the left neighbor is missing, assign -1.
        if missing_left:
            gap_dict[i] = -1
        # If only the right neighbor is missing, assign 1.
        elif missing_right:
            gap_dict[i] = 1
...
def analyze_is_helix_in_gap(chain: Chain, pdb_id: str, source_tag: str, gap_residue_dict: Dict[int, int]) -> Dict[int, bool]:
    """
    Analyzes whether the neighboring residues adjacent to gap positions are helices.
    
    The function writes the given chain to a temporary file and runs DSSP on it. For each residue number 
    in `gap_residue_dict` (which maps a gap residue to a gap direction indicator, -1 for left and 1 for right), 
    the function:
      1. Constructs a DSSP key based on the provided chain ID and residue number.
      2. Checks whether the residue at that gap position is assigned as a helix ('H') by DSSP.
      3. If the gap residue is helix, the neighboring residue (offset by the gap direction) is checked.
      4. If the neighbor exists in DSSP data, its helix assignment is recorded.
    
    If a key (either for the gap residue or its neighbor) is missing from the DSSP output, a message is printed 
    and the residue is skipped.
    
    Args:
        chain (Chain): A Bio.PDB.Chain object.
        pdb_id (str): PDB identifier.
        source_tag (str): A tag indicating the source of the chain.
        gap_residue_dict (Dict[int, int]): Dictionary mapping gap residue numbers to gap direction 
                                           (-1 for left missing neighbor, 1 for right missing neighbor).
    
    Returns:
        Dict[int, bool]: Dictionary mapping each gap residue number to a boolean value indicating 
                         whether the neighboring residue (in the gap direction) is assigned as a helix.
    """
    # Write the chain to a temporary file and retrieve the temp file and directory.
    ...
    chain_id = chain.id  # use the actual chain ID
    
    for resnum, gap_direction in gap_residue_dict.items():
        # Construct the DSSP key for the gap residue.
        gap_key = (chain_id, (' ', resnum, ' '))
        if gap_key not in dssp:
            print(f"Residue {resnum} not found in DSSP data for chain {chain_id}.")
            continue
        
        # Only proceed if the gap residue is a helix.
        if dssp[gap_key][2] == 'H':
            neighbor_resnum = resnum + gap_direction
            neighbor_key = (chain_id, (' ', neighbor_resnum, ' '))
            if neighbor_key in dssp:
                is_helix = dssp[neighbor_key][2] == 'H'
                result[resnum] = is_helix
            else:
                print(f"Neighbor residue {neighbor_resnum} not found in DSSP data for chain {chain_id}.")
